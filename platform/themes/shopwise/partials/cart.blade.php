            ...
            @foreach(Cart::instance('cart')->content() as $key => $cartItem)
                ...
                @if (!empty($product))
                    <li>
                        ...
                        <a href="{{ $product->original_product->url }}"><img src="{{ $cartItem->options['image'] }}" alt="{{ $product->name }}" /> {{ $product->name }}  @if ($product->isOutOfStock()) <span class="stock-status-label">({!! $product->stock_status_html !!})</span> @endif</a>
